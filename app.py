...
<head>
    ...
    <style>
        ...
        /* Photorealistic 3D Viewport mimicking dusk urban environments */
        #gameArea { 
            position: relative; width: 380px; height: 480px; 
            background: linear-gradient(to bottom, #1f2d3d 0%, #3a4d5e 40%, #1f2530 41%, #0f131a 100%); 
        ...
        /* Chapter 1: European City streets architecture with deep window shading */
        .city-facade-l { position: absolute; top: 20px; left: 0; width: 125px; height: 180px; background: linear-gradient(135deg, #3d3a37, #242220); border-right: 3px solid #111; box-shadow: inset -5px 0 15px rgba(0,0,0,0.5), 12px 0 25px rgba(0,0,0,0.6); }
        .city-facade-r { position: absolute; top: 15px; right: 0; width: 115px; height: 185px; background: linear-gradient(225deg, #363330, #1f1d1b); border-left: 3px solid #111; box-shadow: inset 5px 0 15px rgba(0,0,0,0.5), -12px 0 25px rgba(0,0,0,0.6); }
        
        /* Overhead trolley / tram wire mesh layout */
        .overhead-wires { 
            ...
            background: repeating-linear-gradient(45deg, transparent, transparent 35px, rgba(255,255,255,0.03) 35px, rgba(255,255,255,0.03) 36px),
                        repeating-linear-gradient(-45deg, transparent, transparent 45px, rgba(255,255,255,0.02) 45px, rgba(255,255,255,0.02) 46px);
            opacity: 0.8; 
        }

        /* Chapter 3: Rain-Slicked Cargo Port Terminal elements */
        .port-crane { position: absolute; top: 10px; left: 15px; width: 85px; height: 140px; background: linear-gradient(to right, #1a222d, #0b0f14); border-radius: 4px; }
        .cargo-vessel { position: absolute; top: 60px; right: -20px; width: 140px; height: 110px; background: linear-gradient(to bottom, #111a24, #05080c); border-radius: 50% 12px 12px 50%; }
        ...
        /* Concrete road featuring dual tram lines embedded in tarmac */
        .roadway { position: absolute; bottom: 0; left: 0; width: 100%; height: 280px; background: linear-gradient(to bottom, #444a52, #24282e); clip-path: polygon(46% 0%, 54% 0%, 100% 100%, 0% 100%); z-index: 2; }
        /* Tram rails layout paths */
        .roadway::before { 
            content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%;
            background: repeating-linear-gradient(to right, transparent, transparent 48%, rgba(255,255,255,0.08) 48%, rgba(255,255,255,0.08) 52%, transparent 52%);
            opacity: 0.4;
        }
        /* 🚗 MILITARY ARMORED PERSONNEL CARRIER (APC) COVER ASSET */
        #car {
            position: absolute; top: 172px; left: 105px; width: 170px; height: 100px;
            background: linear-gradient(to bottom, #556b2f 0%, #3b4d1a 45%, #1e2908 100%); border-radius: 6px;
            box-shadow: 0 25px 45px rgba(0,0,0,0.8), inset 0 2px 4px rgba(255,255,255,0.1); z-index: 4;
            border: 2px solid #28360c; will-change: transform, left, top; transform-origin: center bottom;
        }
        /* Tactical vehicle windshield slits */
        .window { position: absolute; top: 16px; left: 20px; width: 130px; height: 22px; background: linear-gradient(180deg, rgba(80,120,160,0.6) 0%, rgba(5,15,30,0.8) 100%); border: 2px solid #000; }
        
        .wheel { position: absolute; background: #121212; border-radius: 3px; border: 1.5px solid #2c2c2c; }
        ...
        /* Dynamic vehicle firing ports replacing side doors */
        .suv-door { position: absolute; top: 45px; width: 20px; height: 35px; background: #28360c; border: 1.5px solid #000; z-index: 5; transition: transform 0.4s ease; }
        ...
        .light-l { position: absolute; bottom: 20px; left: 10px; width: 16px; height: 12px; background: #ffea00; box-shadow: 0 0 15px #ffea00; }
        .light-r { position: absolute; bottom: 20px; right: 10px; width: 16px; height: 12px; background: #ffea00; box-shadow: 0 0 15px #ffea00; }

        /* 🔫 CENTERING TACTICAL HOLO SIGHT GUN CODES */
        ...
        .w-slide { position: absolute; top: 40px; left: 24px; width: 52px; height: 50px; background: linear-gradient(to right, #1a1a1a 0%, #3a3a3a 30%, #151515 50%, #3a3a3a 70%, #1a1a1a 100%); border-radius: 6px 6px 2px 2px; border-top: 1px solid #555; box-shadow: 0 10px 25px rgba(0,0,0,0.7), inset 0 2px 4px rgba(255,255,255,0.15); }
        .w-holo-sight { position: absolute; top: 2px; left: 29px; width: 42px; height: 38px; border: 3.5px solid #222; border-bottom: none; border-radius: 6px 6px 0 0; background: linear-gradient(to bottom, rgba(0,240,255,0.12), rgba(0,240,255,0.03)); box-shadow: inset 0 0 8px rgba(0,240,255,0.2); }
        ...
        /* 🏃 UPGRADED CAMOUFLAGE MILITARY SOLDIER THREAT LAYOUTS */
        ...
        .t-head { background: linear-gradient(135deg, #dfa36b, #b57a53); border-radius: 50%; width: 24px; height: 24px; border: 1.5px solid #111; position: relative; }
        /* Woodland pattern ballistic helmet overlay */
        .t-head::before { content: ''; position: absolute; top: -3px; left: -1px; width: 26px; height: 12px; background: repeating-linear-gradient(45deg, #3d4a26, #3d4a26 4px, #242b15 4px, #242b15 8px); border-radius: 6px 6px 0 0; border: 1px solid #111; } 
        ...
        /* Heavy load-bearing tactical armor vest fabric shading */
        .t-torso { background: repeating-linear-gradient(to right, #445233, #5e6b40 6px, #2d381c 12px); width: 34px; height: 38px; border-radius: 4px; border: 1.5px solid #111; position: relative; box-shadow: 0 5px 8px rgba(0,0,0,0.4); }
        .t-arm { position: absolute; top: 6px; width: 26px; height: 11px; background: #445233; border: 1.5px solid #111; border-radius: 3px; }
        ...
        .t-weapon { position: absolute; top: -2px; width: 15px; height: 11px; background: #1a1a1a; border-radius: 2px; border-bottom: 2px solid #000; }
        ...
        .t-leg { background: #2d381c; width: 9px; height: 100%; border-radius: 2px; border: 1px solid #000; animation: walkCycle 0.22s ease-in-out infinite alternate; }
...
<script>
    ...
    const mapChapters = {
        1: { name: "CITY STREETS", bg: "linear-gradient(to bottom, #1f2d3d 0%, #3a4454 40%, #1f2530 41%, #0f131a)", road: "none", code: "city" },
