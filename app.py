...
st.title("存放 Virtua Arcade: Campaign Mode")
...
<head>
    ...
    <style>
        ...
        #gameArea { position: relative; width: 380px; height: 480px; background: linear-gradient(to bottom, #4a777a 0%, #a1c4fd 40%, #727d8c 41%, #3a4454 100%); border: 4px solid #444; overflow: hidden; margin: auto; border-radius: 16px; touch-action: none; box-shadow: 0 12px 32px rgba(0,0,0,0.6); transition: background 0.8s ease; }
        ...
        #car { position: absolute; top: 175px; left: 110px; width: 160px; height: 105px; background: linear-gradient(to bottom, #333, #1a1a1a, #0d0d0d); border-radius: 14px 14px 6px 6px; box-shadow: 0 15px 25px rgba(0,0,0,0.6), inset 0 2px 4px rgba(255,255,255,0.15); z-index: 4; border: 1px solid #222; will-change: transform, left, top; transform-origin: center bottom; }
        .window { position: absolute; top: 12px; left: 15px; width: 130px; height: 35px; background: linear-gradient(180deg, rgba(142, 202, 230, 0.6) 0%, rgba(33, 158, 188, 0.3) 70%, rgba(2, 48, 71, 0.5) 100%); border-radius: 8px 8px 2px 2px; border: 2px solid #000; }
        ...
        #weapon { position: absolute; bottom: -35px; right: 45px; width: 90px; height: 180px; transform: rotate(0deg); transform-origin: bottom center; pointer-events: none; z-index: 25; will-change: transform; }
        ...
        .w-slide::before { content: ''; position: absolute; left: -4px; top: 6px; width: 4px; height: 10px; background: #000; border-radius: 1px; }
        .w-grip { position: absolute; top: 34px; left: 25px; width: 34px; height: 110px; background: repeating-linear-gradient(45deg, #3d2b1f, #3d2b1f 2px, #241911 2px, #241911 4px); border: 3px solid #0a0a0a; border-radius: 4px 6px 12px 6px; transform: rotate(-10deg); box-shadow: -6px 6px 12px rgba(0,0,0,0.6); }
        ...
        .t-torso { background: linear-gradient(to right, #1d3557, #457b9d, #1d3557); width: 32px; height: 36px; border-radius: 4px; border: 1.5px solid #111; position: relative; box-shadow: 0 4px 8px rgba(0,0,0,0.4); }
        ...
        .t-weapon { position: absolute; top: -2px; width: 14px; height: 11px; background: linear-gradient(to bottom, #444, #111); border-radius: 2px; border-bottom: 2px solid #000; }
        ...
        .blood-drop { position: absolute; width: 4px; height: 4px; background: #ba000d; border-radius: 50%; z-index: 12; pointer-events: none; animation: explodeBlood 0.35s ease-out forwards; }
...
<body>
    <div id="gameArea">
        ...
        <div id="overScreen">
            <div style="color:#ffea00; font-size:32px; font-weight:bold; font-family:monospace; text-shadow:0 0 8px red;">OUTPOST OVERRUN</div>
            <div id="finalScore" style="color:white; font-size:18px; margin-top:10px;">Final Arcade Score: 200</div>
            <button class="retry-btn" onclick="resetArcadeEngine(true)">REDEPLOY FROM CH. 1 🔄</button>
        </div>

        <div id="winScreen">
            <div style="color:#ffea00; font-size:28px; font-weight:bold; text-align:center; text-shadow: 0 0 10px #ffea00;">👑 CAMPAIGN VICTORY 👑</div>
            <div style="color:white; font-size:16px; text-align:center; margin-top:15px; max-width:300px; line-height:1.5;">CONGRATULATIONS OFFICER!<br>You have successfully completed all 20 Chapters and secured the city!</div>
    ...
    function spawnBloodSpit(x, y) {
        for (let i = 0; i < 8; i++) {
            ...
            let vx = (Math.random() * 40 - 20); let vy = (Math.random() * -35 - 5);
    ...
    function triggerFire() {
        ...
        threatsList.forEach((t, index) => {
            ...
            let tW = tRect.width; let tH = tRect.height;
            ...
            if (hitCenterX >= tX && hitCenterX <= tX + tW && hitCenterY >= tY && hitCenterY <= tY + tH) {
                ...
                t.ring.remove();
                t.el.style.transform += " rotate(80deg)"; t.el.style.top = (parseFloat(t.el.style.top) + 20) + "px"; t.el.style.opacity = "0";

                setTimeout(() => { t.el.remove(); threatsList = threatsList.filter(item => item !== t); }, 400);

                if (chapterKills >= maxNeeded) {
                    if (activeChapter >= 20) { isOver = true; clearInterval(spawnTimerId); clearInterval(physicsTimerId); winScreen.style.display = "flex"; return; }
                    sound("level"); activeChapter += 1; chapterKills = 0; chapterTxt.innerText = "MISSION ACCOMPLISHED! 🎉";
                    setTimeout(() => { updateLevelAtmosphere(); resetArcadeEngine(false); }, 1200);
                }
            }
        });
    }
    function runEngineLoops() {
        physicsTimerId = setInterval(() => {
            if (isOver) return;
            if (!carParked) { distanceScale += 0.015; if (distanceScale >= 1.0) { distanceScale = 1.0; carParked = true; } }
            ...
            threatsList.forEach((t) => {
                if (t.isDying) return;
                let updatedX = carPos + (t.sideOffset * distanceScale); let threatY = currentTopY + (t.baseTopY - 195) * distanceScale;
                t.el.style.transform = "scale(" + distanceScale + ")"; t.el.style.left = updatedX + "px"; t.el.style.top = threatY + "px";
                t.ring.style.width = (90 * (1.3 - (t.age / 40))) + "px"; t.ring.style.height = (90 * (1.3 - (t.age / 40))) + "px";
                let rSize = 90 * (1.3 - (t.age / 40)); t.ring.style.left = (updatedX + (20 * distanceScale) - (rSize / 2) + 15) + "px"; t.ring.style.top = (threatY + (15 * distanceScale) - (rSize / 2) + 30) + "px"; t.age += 1;
            });
        }, 30);
        ...
        spawnTimerId = setInterval(() => {
            ...
            let updatedX = carPos + (sideOffset * distanceScale); let threatY = (165 + (distanceScale * 45)) + (topY - 195) * distanceScale;
            el.style.left = updatedX + "px"; el.style.top = threatY + "px"; el.style.transform = "scale(" + distanceScale + ")"; gameArea.appendChild(el);

            let ring = document.createElement("div"); ring.className = "target-ring"; gameArea.appendChild(ring);
            let threatObj = { el: el, ring: ring, sideOffset: sideOffset, baseTopY: topY, age: 0, isDying: false }; threatsList.push(threatObj);
            sound("ding");

            setTimeout(() => { if (!isOver && el.parentNode && !threatObj.isDying) { isOver = true; sound("boom"); clearInterval(spawnTimerId); clearInterval(physicsTimerId); finalScore.innerText = "Final Arcade Score: " + score; overScreen.style.display = "flex"; } }, 1400);
        }, 1100);
    }

    window.resetArcadeEngine = function(resetFullCampaign) {
        clearInterval(spawnTimerId); clearInterval(physicsTimerId);
        threatsList.forEach(t => { t.el.remove(); t.ring.remove(); }); threatsList = [];
        if (resetFullCampaign) { score = 200; activeChapter = 1; chapterKills = 0; scoreCounter.innerText = "00200"; }
        updateLevelAtmosphere(); isOver = false; distanceScale = 0.2; carParked = false; carPos = Math.random() * 80 + 70;
        sight.style.left = "168px"; sight.style.top = "218px"; weapon.style.transform = "rotate(0deg) translateX(0px) translateY(0px)"; overScreen.style.display = "none"; winScreen.style.display = "none";
        runEngineLoops();
    };
